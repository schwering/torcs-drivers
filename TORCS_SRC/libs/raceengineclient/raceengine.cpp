...
reCapture(void)
{
    ...
    // Skip half of frames which makes the video effectively 25fps/2 = 12.5fps.
    if (++ctr % skip != 0) {
      return;
    }
    ...
    sprintf(buf, "%s/torcs-%4.4d-%8.8d.pnm", capture->outputBase, capture->currentCapture, capture->currentFrame++);
    GfImgWritePnm(img, buf, (size_t) vw, (size_t) vh);
    free(img);
}
