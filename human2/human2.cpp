...
/***************************************************************************
    ...
    email                : torcs@free.fr
    version              : $Id: human2.cpp,v 1.45.2.4 2008/11/09 20:14:35 berniw Exp $
...
static tdble lastSpeedLimiterCommand = -10.0;
...
human2(tModInfo *modInfo)
{
	...
	snprintf(buf, BUFSIZE, "%sdrivers/human2/human2.xml", GetLocalDir());
 ...
 *
 * Description
 *	search under drivers/human2/tracks/<trackname>/car-<model>-<index>.xml
 *		     drivers/human2/car-<model>-<index>.xml
 *		     drivers/human2/tracks/<trackname>/car-<model>.xml
 *		     drivers/human2/car-<model>.xml
 *
 * Parameters
 *
 *
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *s)
{
	...
	if (s2-s1 < TRACKNAMESIZE) {
		...
		trackname[0] = 0;
		GfError("human2.cpp, initTrack, filename too long");
	}
	...
	snprintf(sstring, BUFSIZE, "%sdrivers/human2/tracks/%s/car-%s-%d.xml", GetLocalDir(), trackname, carname, index);
	*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
	if (*carParmHandle != NULL) {
		GfOut("Player: %s Loaded\n", sstring);
	} else {
		snprintf(sstring, BUFSIZE, "%sdrivers/human2/tracks/%s/car-%s.xml", GetLocalDir(), trackname, carname);
		*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
		if (*carParmHandle != NULL) {
			GfOut("Player: %s Loaded\n", sstring);
		} else {
			snprintf(sstring, BUFSIZE, "%sdrivers/human2/car-%s-%d.xml", GetLocalDir(), carname, index);
			*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
			if (*carParmHandle != NULL) {
				GfOut("Player: %s Loaded\n", sstring);
			} else {
				snprintf(sstring, BUFSIZE, "%sdrivers/human2/car-%s.xml", GetLocalDir(), carname);
				*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
				if (*carParmHandle != NULL) {
					GfOut("Player: %s Loaded\n", sstring);
				} else {
					snprintf(sstring, BUFSIZE, "%sdrivers/human2/car.xml", GetLocalDir ());
					*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
					if (*carParmHandle != NULL) {
						GfOut("Player: %s Loaded\n", sstring);
					}
...
static void common_drive(int index, tCarElt* car, tSituation *s)
{
	...
	char sstring[BUFSIZE];
        ...
        /* XXX:chs DA stuff for the different scenarios.
         * Can be flipped using the `light' key (which normally switches on/off
         * the lights of the car. */
#define SPEED_LIMITED   0
#define ACCEL_LIMITED   1
#define UNLIMITED       2
#define N_DRIVING_MODES 3
	...
	{
                if (lastSpeedLimiterCommand + 0.5 < s->currentTime) {
                    speedLimiter = !speedLimiter;
                    lastSpeedLimiterCommand = s->currentTime;
                    fprintf(stderr, "Flipped speedlimiter %s\n", speedLimiter ? "on" : "off");
                }
		//speedLimiter = true;
	...
	{
		drivingMode = (drivingMode + 1) % N_DRIVING_MODES;
	}
	if (((cmd[CMD_ASR].type == GFCTRL_TYPE_JOY_BUT) && joyInfo->edgeup[cmd[CMD_ASR].val]) ||
		((cmd[CMD_ASR].type == GFCTRL_TYPE_KEYBOARD) && keyInfo[cmd[CMD_ASR].val].edgeUp) ||
		((cmd[CMD_ASR].type == GFCTRL_TYPE_SKEYBOARD) && skeyInfo[cmd[CMD_ASR].val].edgeUp))
	{
		drivingMode = (drivingMode - 1);
                if (drivingMode == -1)
                    drivingMode = N_DRIVING_MODES - 1;
	}

        const char *drivingModeStr;
        switch (drivingMode) {
            case SPEED_LIMITED: drivingModeStr = "speed limited"; break;
            case ACCEL_LIMITED: drivingModeStr = "accel limited"; break;
            case UNLIMITED:     drivingModeStr = "unlimited";     break;
            default:            drivingModeStr = "invalid";       break;
        }
	...
	switch (cmd[CMD_LEFTSTEER].type) {
		case GFCTRL_TYPE_JOY_AXIS:
			...
			leftSteer = -SIGN(ax0) * cmd[CMD_LEFTSTEER].pow * pow(fabs(ax0), cmd[CMD_LEFTSTEER].sens) / (1.0 + cmd[CMD_LEFTSTEER].spdSens * car->_speed_x);
			//leftSteer = -SIGN(ax0) * 0.3 * pow(fabs(ax0), cmd[CMD_LEFTSTEER].sens) / (1.0 + cmd[CMD_LEFTSTEER].spdSens * car->_speed_x);
#if 0
                        fprintf(stderr, "ax0 = %lf\n", ax0);
                        fprintf(stderr, "pow = %lf\n", cmd[CMD_LEFTSTEER].pow);
                        fprintf(stderr, "sens = %lf\n", cmd[CMD_LEFTSTEER].sens);
                        fprintf(stderr, "spdSens = %lf\n", cmd[CMD_LEFTSTEER].spdSens);
                        fprintf(stderr, "leftSteer = %lf\n", leftSteer);
#endif
			break;
	...
	switch (cmd[CMD_RIGHTSTEER].type) {
		case GFCTRL_TYPE_JOY_AXIS:
			...
			rightSteer = -SIGN(ax0) * cmd[CMD_RIGHTSTEER].pow * pow(fabs(ax0), cmd[CMD_RIGHTSTEER].sens) / (1.0 + cmd[CMD_RIGHTSTEER].spdSens * car->_speed_x);
			//rightSteer = -SIGN(ax0) * 0.3 * pow(fabs(ax0), cmd[CMD_RIGHTSTEER].sens) / (1.0 + cmd[CMD_RIGHTSTEER].spdSens * car->_speed_x);
#if 0
                        fprintf(stderr, "ax0 = %lf\n", ax0);
                        fprintf(stderr, "pow = %lf\n", cmd[CMD_LEFTSTEER].pow);
                        fprintf(stderr, "sens = %lf\n", cmd[CMD_LEFTSTEER].sens);
	...
	if (HCtx[idx]->ParamAsr) 
	{
        ...
        if (drivingMode == ACCEL_LIMITED) {
            // Tipp: take slow cars with acceleration limit
            static double lastTime = 0.0;
            static tdble lastSpeed = 0.0;
            static int lastGear = 0;
            static tdble minAccel = 0.0;
            static tdble maxAccel = 1.0;

            const double time = s->currentTime;
            const tdble speed = car->_speed_x;
            const int gear = car->_gear;

            //const tdble goalAccel = (car->_gear <= 2) ? 3.0 - (car->_gear - 1) : 0.0;
            const tdble goalAccel = 3.0 - (car->_gear - 1);

            if (gear != lastGear) {
                printf("%% shifted from %d to %d at speed %lf m/s\n", lastGear, gear, speed);
            }

            if (car->_accelCmd != 0 && lastTime != 0 && time > 0) {
                const tdble accel = (speed - lastSpeed) / (time - lastTime);
                const tdble cmd = (minAccel + maxAccel) / 2;
                if (accel < goalAccel) {
                    minAccel = (maxAccel + minAccel) / 2;
                }
                if (accel > goalAccel) {
                    maxAccel = (maxAccel + minAccel) / 2;
                }
                const tdble delta = 0.02;
                if (maxAccel - minAccel < delta) {
                    maxAccel += delta / 2.0;
                    minAccel -= delta / 2.0;
                }
                car->_accelCmd = cmd;
